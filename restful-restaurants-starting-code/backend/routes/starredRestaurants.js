...
router.get("/:id", (req, res) => {
  ...
  const restaurantById = STARRED_RESTAURANTS.find((restaurant) => {
    restaurant.id == id;
  });
...
router.delete("/:id", (req, res) => {
  ...
  const removedResturant = STARRED_RESTAURANTS.filter(
    (restaurant) => restaurant.id != id
  );

  console.log(removedResturant);
  if (removedResturant.length == STARRED_RESTAURANTS.length) {
    res.sendStatus(404);
  }

  STARRED_RESTAURANTS = removedResturant;
